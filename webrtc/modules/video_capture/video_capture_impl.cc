    ...
    int64_t captureTime/*=0*/)
{
    ...
    if (frameInfo.codecType == kVideoCodecUnknown)
    {
        ...
        {
            printf( "Wrong incoming frame length.");
        }
        ...
        if (ret < 0)
        {
            printf( "Failed to create empty frame, this should only\n");
        }
        const int conversionResult = ConvertToI420(commonVideoType,
                                                   ...
                                                   _rotateFrame,
                                                   &_captureFrame);
	printf("==============yanghang run third,size:%d\n",videoFrameLength);
        if (conversionResult < 0)
        {
           printf("Failed to convert capture frame from type \n");
        }
