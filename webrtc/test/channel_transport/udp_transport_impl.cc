                                           ...
                                           const SocketAddress& to)
{
    ...
    if(_ptrSendRtcpSocket)
    {
        printf("_ptrSendRtcpSocket,port:%d",htons(int(to._sockaddr_in.sin_port)));
        return _ptrSendRtcpSocket->SendTo(data,length,to);

    } else if(_ptrRtcpSocket)
    {
        printf("_ptrRtcpSocket,port:%d",htons(int(to._sockaddr_in.sin_port)));
...
int UdpTransportImpl::SendPacket(int /*channel*/, const void* data, int length)
{
 ...
 configured");
        ...
        _localPort = _destPort;
        printf("_localIP %s,_localPort %d\n",_localIP,_localPort);
        ErrorCode retVal = BindLocalRTPSocket();
        if(retVal != kNoSocketError)
        {
            printf( "SendPacket() failed to bind RTP socket\n");
            _lastError = retVal;
            CloseReceiveSockets();
            return -1;
        }
    }
    printf("udp_transport_impl.cc\n");
    if(_ptrSendRtpSocket)
    {
	printf("ptrSendRtpSocket\n");
        return _ptrSendRtpSocket->SendTo((const int8_t*)data, length,
                                         _remoteRTPAddr);

    } else if(_ptrRtpSocket)
    {
	printf("_ptrRtpSocket\n");
                                     ...
                                     int length)
{
 ...
 configured");
    ...
    if(_ptrSendRtcpSocket)
    {
        printf("_ptrSendRtcpSocket length=%d\n",length);
        return _ptrSendRtcpSocket->SendTo((const int8_t*)data, length,
                                          _remoteRTCPAddr);
    } else if(_ptrRtcpSocket)
    {
        printf("_ptrRtcpSocket length=%d\n",length);
                                            ...
                                            const SocketAddress* fromSocket)
{
 ...
 information");
        ...
        printf("1_Incoming RTCP packet from ip:%s port:%d\n", ipAddress,portNr);
        // Filter based on port.
 ...
 allowed port:%d",
    ...
    if (_packetCallback)
    {
        printf("2_Incoming RTCP packet from ip:%s port:%d\n", ipAddress,portNr);
