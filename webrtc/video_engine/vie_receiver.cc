...
bool ViEReceiver::OnRecoveredPacket(const uint8_t* rtp_packet,
  ...
  printf("Enter OnRecoveredPacket Function\n");
...
int ViEReceiver::InsertRTPPacket(const uint8_t* rtp_packet,
  ...
  printf("Enter InsertRTPPacket Function\n");
  RTPHeader header;
  if (!rtp_header_parser_->Parse(rtp_packet, rtp_packet_length,
                                 &header)) {
    return -1;
  }
  printf("rtp sequenceNumber=%d,timestamp=%d,packet_size=%d\n",header.sequenceNumber,header.timestamp,rtp_packet_length);
  static int discard_num = 0;
  discard_num++;
  if(discard_num == 20)
  {
    discard_num = 0;
    printf("discard the the rtp packet,its seq_num %d\n",header.sequenceNumber);
    return 0;
  }
...
bool ViEReceiver::ParseAndHandleEncapsulatingHeader(const uint8_t* packet,
  ...
  if (rtp_payload_registry_->IsRed(header)) {
    printf("################Recevie Red Packet,seq_num %d\n",header.sequenceNumber);
    ...
    printf("@@@@@@@@@@@@@Recevie RTX Packet,seq_num %d\n",header.sequenceNumber);
