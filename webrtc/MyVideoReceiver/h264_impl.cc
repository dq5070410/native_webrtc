...
namespace webrtc {
	...
	int H264EncoderImpl::InitEncode(const VideoCodec* codecSettings, int32_t numberOfCores, uint32_t maxPayloadSize) {
		if (codecSettings == NULL) {
			return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
		}
		if (codecSettings->maxFramerate < 1) {
			return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
		}
		// allow zero to represent an unspecified maxBitRate
		if (codecSettings->maxBitrate > 0 && codecSettings->startBitrate > codecSettings->maxBitrate) {
			return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
		}
		...
		param.iTargetBitrate = codecSettings->maxBitrate;
		...
		param.i_width = codecSettings->width;
		param.i_height = codecSettings->height;
		param.b_vfr_input = 0;
		...
		WEBRTC_TRACE(webrtc::kTraceApiCall, webrtc::kTraceVideoCoding, -1,
			...
			codecSettings->width, codecSettings->height, codecSettings->maxFramerate, codecSettings->startBitrate, codecSettings->maxBitrate);
	...
	int H264EncoderImpl::Encode(const I420VideoFrame& input_image,
		...
		printf("enter external x264 encoder\n");
		if (!inited_) {
			return WEBRTC_VIDEO_CODEC_UNINITIALIZED;
		}
...
#endif
	{
		...
		fp_h264=fopen("output_264.h264","wb+");
#endif
	}
	...
	int H264DecoderImpl::InitDecode(const VideoCodec* codecSettings, int number_of_cores) {
		if (codecSettings == NULL) {
			return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
		}
		int ret_val = Release();
		...
		if (&codec_ != codecSettings) {
			// Save VideoCodec codecSettingsance for later; mainly for duplicating the decoder.
			codec_ = *codecSettings;
		}
#if USEOPENH264
		if (decoder_ == NULL) {
	...
	int H264DecoderImpl::Decode(const EncodedImage& input_image,
		...
		printf("enter external decode x264\n");
		fwrite(input_image._buffer,1,input_image._length,fp_h264);
		if (!inited_) {
			printf("H264DecoderImpl::Decode, decoder is not initialized");
			return WEBRTC_VIDEO_CODEC_UNINITIALIZED;
		}

		if (decode_complete_callback_ == NULL) {
			printf("H264DecoderImpl::Decode, decode complete call back is not set");
			return WEBRTC_VIDEO_CODEC_UNINITIALIZED;
		}

		if (input_image._buffer == NULL) {
			printf("H264DecoderImpl::Decode, null buffer");
			return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
		}
		if (!codec_specific_info) {
			printf("H264EncoderImpl::Decode, no codec info");
			return WEBRTC_VIDEO_CODEC_ERROR;
		}
		if (codec_specific_info->codecType != kVideoCodecH264) {
			printf("H264EncoderImpl::Decode, non h264 codec %d", codec_specific_info->codecType);
			return WEBRTC_VIDEO_CODEC_ERROR;
		}

		printf("H264DecoderImpl::Decode(frame_type:%d, length:%d)\n",
		...
		if (rv != dsErrorFree) {
			printf("H264DecoderImpl::Decode, openH264 decoding fails with error %d", rv);
			return WEBRTC_VIDEO_CODEC_ERROR;
		}
		...
		if (buffer_info.iBufferStatus == 1) {
			...
			printf("H264DecoderImpl::Decode, buffer status:%d", buffer_info.iBufferStatus);
			return WEBRTC_VIDEO_CODEC_OK;
		}
#elif USEX264	
		if (framecnt < 2)
		...
		else
		{
			...
			if (ret < 0){
				printf("H264DecoderImpl::Decode, Decode Error.");
				return WEBRTC_VIDEO_CODEC_ERROR;
			}
			if (got_picture){
